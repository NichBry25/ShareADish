import { FC } from "react";

type StarRatingProps = {
  value: number;
  max?: number;
};

const starClasses = "h-4 w-4";

export const StarRating: FC<StarRatingProps> = ({ value, max = 5 }) => {
  const filled = Math.round(value);
  return (
    <div className="flex items-center gap-1" aria-label={`${value} out of ${max} stars`}>
      {Array.from({ length: max }).map((_, index) => {
        const isFilled = index < filled;
        return (
          <svg
            key={index}
            className={`${starClasses} ${isFilled ? "text-amber-400" : "text-zinc-300"}`}
            viewBox="0 0 20 20"
            fill="currentColor"
            aria-hidden="true"
          >
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.802 2.036a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.802-2.036a1 1 0 00-1.176 0l-2.802 2.036c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        );
      })}
      <span className="text-xs font-medium text-zinc-500">{value.toFixed(1)}</span>
    </div>
  );
};
