...
export default function RecipeEdit({recipe, onSubmit}:RecipeEditProps) {
  ...
  const handleGenerate = async (event: FormEvent<HTMLFormElement>) => {
      ...
      if (!prompt.trim()) {
        setErrorMessage("Please enter a prompt so we know what to change.");
        return;
      }
  ...
  return (
    <main className="min-h-screen bg-zinc-50 pb-24">
      ...
      <section className="mx-auto w-full max-w-6xl px-6 py-3">
        <h1 className="mt-8 text-2xl font-semibold text-neutral-900">Edit Recipe</h1>
        <div className="mt-8 grid gap-8 lg:grid-cols-[4fr_2fr]">
          ...
          >
            <div className="flex flex-col">
              <h2 className="text-lg font-semibold text-neutral-900">Edit Your Dish</h2>
              <textarea
                id="recipePrompt"
                name="recipePrompt"
                value={prompt}
                onChange={(event) => setPrompt(event.target.value)}
                placeholder="Example: I want the dish to be sweet, savory, or healthy."
